# ATYA-093 — Classify every failed or stale CI run
...
Failed and stale checks obscure whether repositories are releasable and whether open work is safe to merge.

## Root cause

Fleet work accumulated across independent repositories without one stable ATYA identity and evidence-controlled retirement process. Failed and stale checks obscure whether repositories are releasable and whether open work is safe to merge.

## Desired outcome

Inventory failing default-branch, development, master, PR, scheduled, and release workflows; group failures by root cause and assign an owning remediation task without rerunning or mutating workflows during the audit.

## Affected scope

Primary owner: **Atya.Roadmap**. Audit-affected repositories or coordination scopes: Atya.Roadmap, AtyaLibraries, publisher. Only the cited findings and their direct acceptance evidence are added; unaffected architecture remains unchanged.
...
2. Inventory failing default-branch, development, master, PR, scheduled, and release workflows
3. group failures by root cause and assign an owning remediation task without rerunning or mutating workflows during the audit.
...
# Task-specific proof: Inventory failing default-branch, development, master, PR, scheduled, and release workflows; group failures by root cause and assign an owning remediation task without rerunning or mutating workflows during the audit.
```
...
You are implementing ATYA-093 for the AtyaLibraries ecosystem. Work independently; do not assume you have read the original analyses or repository audit.

Task objective

Classify every failed or stale CI run. Inventory failing default-branch, development, master, PR, scheduled, and release workflows; group failures by root cause and assign an owning remediation task without rerunning or mutating workflows during the audit.

Exact repository/package scope

Owning repository: Atya.Roadmap. Primary owner: **Atya.Roadmap**. Audit-affected repositories or coordination scopes: Atya.Roadmap, AtyaLibraries, publisher. Only the cited findings and their direct acceptance evidence are added; unaffected architecture remains unchanged.
...
Root cause: Fleet work accumulated across independent repositories without one stable ATYA identity and evidence-controlled retirement process. Failed and stale checks obscure whether repositories are releasable and whether open work is safe to merge.
