# ATYA-017 — Remove the NuGet template's second publishing path
...
This issue tracks **Remove the NuGet template's second publishing path**. Only the minimum public coordination information is included here. Verified details, affected trust-boundary evidence, and any reproduction material are retained privately by organization maintainers.

Private evidence reference: AUDIT-SEC-017
...
3. Implement the smallest dependency-ordered remediation in **Atya.Templates.NuGetPackage**.
...
Implement ATYA-017 for the AtyaLibraries ecosystem in repository **Atya.Templates.NuGetPackage**. Respect hard dependencies (ATYA-002), obtain AUDIT-SEC-017 through the private maintainer channel, and do not publish private evidence, credentials, logs, or exploit reproduction. Make a focused change with tests, migration notes, rollback guidance, and sanitized completion evidence.
