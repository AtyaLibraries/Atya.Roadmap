# ATYA-091 — Refresh and verify every unmerged-commit classification before action
...
Deleting or consolidating branches without commit-level reachability evidence can discard relevant work.

## Root cause

Fleet work accumulated across independent repositories without one stable ATYA identity and evidence-controlled retirement process. Deleting or consolidating branches without commit-level reachability evidence can discard relevant work.

## Desired outcome

For each non-master/development branch, compute commits and tree changes not reachable from both long-lived branches, classify their value and supersession state, and attach reproducible commands and commit SHAs.
...
2. For each non-master/development branch, compute commits and tree changes not reachable from both long-lived branches, classify their value and supersession state, and attach reproducible commands and commit SHAs.
...
# Task-specific proof: For each non-master/development branch, compute commits and tree changes not reachable from both long-lived branches, classify their value and supersession state, and attach reproducible commands and commit SHAs.
```
...
You are implementing ATYA-091 for the AtyaLibraries ecosystem. Work independently; do not assume you have read the original analyses or repository audit.

Task objective

Refresh and verify every unmerged-commit classification before action. For each non-master/development branch, compute commits and tree changes not reachable from both long-lived branches, classify their value and supersession state, and attach reproducible commands and commit SHAs.
...
Root cause: Fleet work accumulated across independent repositories without one stable ATYA identity and evidence-controlled retirement process. Deleting or consolidating branches without commit-level reachability evidence can discard relevant work.
...
Do not start mutation until these hard dependencies are accepted: ATYA-090.
